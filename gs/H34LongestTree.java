...
class H34LongestTree {
	...
	public static Integer largestTree(final Map<Integer, Integer> immediateParent) {
		...
		//make this map
		for(Map.Entry<Integer, Integer> e : immediateParent.entrySet()) {
			if(parentToChild.containsKey(e.getValue())) {
				List<Integer> p = parentToChild.get(e.getValue());
				p.add(e.getKey());
				parentToChild.replace(e.getValue(), p);
			}
			else {
				List<Integer> p = new ArrayList<Integer>();
				p.add(e.getKey());
				parentToChild.put(e.getValue(), p);
			}
		}
		//for each key(parent) call getTreeSize
		//keep tab of max size, if size is same root should be small
		int max = 0;
		int rootMax = -1;
		for(Map.Entry<Integer, List<Integer>> e : parentToChild.entrySet()) {
			rootMax = (max < getTreeSize(parentToChild, e.getKey())) ? e.getKey() : rootMax;
			max = (max < getTreeSize(parentToChild, e.getKey())) ? getTreeSize(parentToChild, e.getKey()) : max;
		}
		return rootMax;
	}
